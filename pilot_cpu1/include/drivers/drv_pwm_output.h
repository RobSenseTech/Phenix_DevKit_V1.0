...
#ifndef DRV_PWM_OUTPUT_H_
#define DRV_PWM_OUTPUT_H_
#include "ioctl.h"
#include <stdint.h>
...
#define pwm_output_values output_pwm_s         //等待uorb移植完成
...
#define PWM_SERVO_ARM		_IOW(_PWM_SERVO_BASE, 0, int)

/** disarm all servo outputs (stop generating pulses) */
#define PWM_SERVO_DISARM	_IOW(_PWM_SERVO_BASE, 1, int)

/** get default servo update rate */
#define PWM_SERVO_GET_DEFAULT_UPDATE_RATE _IOR(_PWM_SERVO_BASE, 2, int)

/** set alternate servo update rate */
#define PWM_SERVO_SET_UPDATE_RATE _IOW(_PWM_SERVO_BASE, 3, int)

/** get alternate servo update rate */
#define PWM_SERVO_GET_UPDATE_RATE _IOR(_PWM_SERVO_BASE, 4, int)

/** get the number of servos in *(unsigned *)arg */
#define PWM_SERVO_GET_COUNT	_IOR(_PWM_SERVO_BASE, 5, int)

/** selects servo update rates, one bit per servo. 0 = default (50Hz), 1 = alternate */
#define PWM_SERVO_SET_SELECT_UPDATE_RATE _IOW(_PWM_SERVO_BASE, 6, int)

/** check the selected update rates */
#define PWM_SERVO_GET_SELECT_UPDATE_RATE _IOR(_PWM_SERVO_BASE, 7, int)

/** set the 'ARM ok' bit, which activates the safety switch */
#define PWM_SERVO_SET_ARM_OK	_IOW(_PWM_SERVO_BASE, 8, int)

/** clear the 'ARM ok' bit, which deactivates the safety switch */
#define PWM_SERVO_CLEAR_ARM_OK	_IOW(_PWM_SERVO_BASE, 9, int)

/** start DSM bind */
#define DSM_BIND_START	_IO(_PWM_SERVO_BASE, 10)
...
#define DSM_BIND_POWER_UP _IO(_PWM_SERVO_BASE, 11)

/** set the PWM value for failsafe */
#define PWM_SERVO_SET_FAILSAFE_PWM	_IOW(_PWM_SERVO_BASE, 12, int)

/** get the PWM value for failsafe */
#define PWM_SERVO_GET_FAILSAFE_PWM	_IOR(_PWM_SERVO_BASE, 13, int)

/** set the PWM value when disarmed - should be no PWM (zero) by default */
#define PWM_SERVO_SET_DISARMED_PWM	_IOW(_PWM_SERVO_BASE, 14, int)

/** get the PWM value when disarmed */
#define PWM_SERVO_GET_DISARMED_PWM	_IOR(_PWM_SERVO_BASE, 15, int)

/** set the minimum PWM value the output will send */
#define PWM_SERVO_SET_MIN_PWM	_IOW(_PWM_SERVO_BASE, 16, int)

/** get the minimum PWM value the output will send */
#define PWM_SERVO_GET_MIN_PWM	_IOR(_PWM_SERVO_BASE, 17, int)

/** set the maximum PWM value the output will send */
#define PWM_SERVO_SET_MAX_PWM	_IOW(_PWM_SERVO_BASE, 18, int)

/** get the maximum PWM value the output will send */
#define PWM_SERVO_GET_MAX_PWM	_IOR(_PWM_SERVO_BASE, 19, int)

/** set the number of servos in (unsigned)arg - allows change of
 * split between servos and GPIO */
#define PWM_SERVO_SET_COUNT	_IOW(_PWM_SERVO_BASE, 20, int)

/** set the lockdown override flag to enable outputs in HIL */
#define PWM_SERVO_SET_DISABLE_LOCKDOWN		_IOW(_PWM_SERVO_BASE, 21, int)   ///////

/** get the lockdown override flag to enable outputs in HIL */
#define PWM_SERVO_GET_DISABLE_LOCKDOWN		_IOR(_PWM_SERVO_BASE, 22, int)

/** force safety switch off (to disable use of safety switch) */
#define PWM_SERVO_SET_FORCE_SAFETY_OFF		_IOW(_PWM_SERVO_BASE, 23, int)

/** force failsafe mode (failsafe values are set immediately even if failsafe condition not met) */
#define PWM_SERVO_SET_FORCE_FAILSAFE		_IOW(_PWM_SERVO_BASE, 24, int)

/** make failsafe non-recoverable (termination) if it occurs */
#define PWM_SERVO_SET_TERMINATION_FAILSAFE	_IOW(_PWM_SERVO_BASE, 25, int)

/** force safety switch on (to enable use of safety switch) */
#define PWM_SERVO_SET_FORCE_SAFETY_ON		_IOW(_PWM_SERVO_BASE, 26, int)

/** set RC config for a channel. This takes a pointer to pwm_output_rc_config */
#define PWM_SERVO_SET_RC_CONFIG			_IOW(_PWM_SERVO_BASE, 27, int)

/** set the 'OVERRIDE OK' bit, which allows for RC control on FMU loss */
#define PWM_SERVO_SET_OVERRIDE_OK		_IOW(_PWM_SERVO_BASE, 28, int)

/** clear the 'OVERRIDE OK' bit, which allows for RC control on FMU loss */
#define PWM_SERVO_CLEAR_OVERRIDE_OK		_IOW(_PWM_SERVO_BASE, 29, int)

/** setup OVERRIDE_IMMEDIATE behaviour on FMU fail */
#define PWM_SERVO_SET_OVERRIDE_IMMEDIATE	_IOW(_PWM_SERVO_BASE, 30, int)

/** set SBUS output frame rate in Hz */
#define PWM_SERVO_SET_SBUS_RATE			_IOW(_PWM_SERVO_BASE, 31, int)
...
#define PWM_SERVO_SET(_servo)	_IOW(_PWM_SERVO_BASE, 0x20 + _servo, int)

/** get a single specific servo value */
#define PWM_SERVO_GET(_servo)	_IOR(_PWM_SERVO_BASE, 0x40 + _servo, int)

/** get the _n'th rate group's channels; *(uint32_t *)arg returns a bitmap of channels
 *  whose update rates must be the same.
 */
#define PWM_SERVO_GET_RATEGROUP(_n) _IOR(_PWM_SERVO_BASE, 0x60 + _n, int)
