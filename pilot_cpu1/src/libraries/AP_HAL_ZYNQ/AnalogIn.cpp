...
static const struct {
	...
	{ 10,   3.3f/4096  },    // 3DR Brick voltage, usually 10.1:1
                            // scaled from battery voltage
    { 11,   3.3f/4096  },    // 3DR Brick current, usually 17:1 scaled
                            // for APM_PER_VOLT
...
void PX4AnalogIn::_timer_tick(void)
{
    ...
    if (ret > 0) {
        ...
        for (uint8_t i=0; i<ret/sizeof(buf_adc[0]); i++) {
			...
			_board_voltage = 5;//robsense的板子去掉了5v电压检测，为了正常解锁这里写死
#endif
        }
        for (uint8_t i=0; i<ret/sizeof(buf_adc[0]); i++) {
...
#if defined(CONFIG_ARCH_BOARD_PX4FMU_V2) || defined(CONFIG_ARCH_BOARD_PX4FMU_V4) || defined(CONFIG_ARCH_BOARD_PX4FMU_ROB)
