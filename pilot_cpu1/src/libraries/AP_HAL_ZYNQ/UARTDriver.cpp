...
/*
  ...
  called in main loop, HAL_PX4_Class
 */

void PX4UARTDriver::begin(uint32_t baudrate, uint16_t rxS, uint16_t txS) 
{
    int ret = 0;
	...
	if (rxS != 0 && rxS != _readbuf_size) {
		...
		_readbuf = (uint8_t *)pvPortMalloc(_readbuf_size);
		_readbuf_head = 0;
		_readbuf_tail = 0;
    }
	...
	if (txS != 0 && txS != _writebuf_size) {
		...
		_writebuf = (uint8_t *)pvPortMalloc(_writebuf_size+16);
		_writebuf_head = 0;
        _writebuf_tail = 0;
	}
...
void PX4UARTDriver::begin(uint32_t baudrate) 
{
	begin(baudrate, 0, 0);
}
...
int PX4UARTDriver::_write_fd(const uint8_t *buf, uint16_t n)
{
    ...
    if (ioctl(_fd, FIONWRITE, (unsigned long)&nwrite) == 0) {
        ...
        if (nwrite > 0) {
            ret = ::write(_fd, (const char *)buf, nwrite);
        }
    }
