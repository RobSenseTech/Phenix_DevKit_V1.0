    ...
    switch (status) {
        ...
        case ACCEL_CAL_WAITING_FOR_ORIENTATION:     
            ...
            if (!running()) {
                _samples_collected = 0;
                if (_sample_buffer == NULL) {
                    _sample_buffer = (struct AccelSample*)pvPortMalloc(_conf_num_samples*sizeof(struct AccelSample));
                    if (_sample_buffer == NULL) {
                        set_status(ACCEL_CAL_FAILED);
                        break;
                    }
                    memset(_sample_buffer, 0, _conf_num_samples*sizeof(struct AccelSample));
                }
            }
            if (_samples_collected >= _conf_num_samples) {
                break;
