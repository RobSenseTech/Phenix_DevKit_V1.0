...
void AP_AccelCal::update()
{
    ...
    if (_started) {
        ...
        if (num_active_calibrators != _num_active_calibrators) {
        	printf("AccelCal fail num_active_calibrators=%d _num_active_calibrators=%d\n", num_active_calibrators, _num_active_calibrators);
            fail();
            return;
        }
        ...
        switch(_status) {
            ...
            case ACCEL_CAL_WAITING_FOR_ORIENTATION: {
                ...
                for(uint8_t i=1 ; (cal = get_calibrator(i))  ; i++) {
                    if (step != cal->get_num_samples_collected()+1) {
                    	printf("AccelCal fail step=%d get_num_samples_collected=%d\n", step, cal->get_num_samples_collected()+1);
                        fail();
                        return;
                    }
                }
            ...
            case ACCEL_CAL_COLLECTING_SAMPLE:
                ...
                if (_status == ACCEL_CAL_FAILED) {
                	printf("AccelCal fail ACCEL_CAL_COLLECTING_SAMPLE\n");
                    fail();
                }
                return;
                ...
                if (_saving) {
                    ...
                    for(uint8_t i=0; i<_num_clients; i++) {
                        if(client_active(i) && _clients[i]->_acal_get_fail()) {
                        	printf("AccelCal fail client_active=%d _acal_get_fail=%d\n", client_active(i), _clients[i]->_acal_get_fail());
                            fail();
                            return;
                        }
                    }
