...
void Copter::esc_calibration_passthrough()
{
    ...
    // clear esc flag for next time
    g.esc_calibrate.set_and_save(ESCCAL_NONE);
	printf("reduce update rate to motors to 50Hz\n");
    // reduce update rate to motors to 50Hz
    motors.set_update_rate(50);

    // send message to GCS
    gcs_send_text(MAV_SEVERITY_INFO,"ESC calibration: Passing pilot throttle to ESCs");
	hal.rcout->set_esc_scaling(channel_throttle->radio_min, channel_throttle->radio_max);//原版代码这里没有，导致校验电调的时候没有pwm输出
    while(1) {
        // arm motors
        motors.armed(true);
        motors.enable();
...
void Copter::esc_calibration_auto()
{
    ...
    while (hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_DISARMED || hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_NONE) {
        if (!printed_msg) {
