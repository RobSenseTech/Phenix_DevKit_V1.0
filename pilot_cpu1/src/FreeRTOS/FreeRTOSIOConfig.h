...
#define ioconfigINCLUDE_SPI									1
	#define ioconfigUSE_SPI_POLLED_TX						1
	#define ioconfigUSE_SPI_POLLED_RX						1
	#define ioconfigUSE_SPI_ZERO_COPY_TX					0
	#define ioconfigUSE_SPI_CIRCULAR_BUFFER_RX				0
...
#if ( ioconfigUSE_SPI == 1 ) && ( ioconfigUSE_SPI_ZERO_COPY_TX == 1 ) && ( ioconfigUSE_ZERO_COPY_TX != 1 )
	#error ioconfigUSE_ZERO_COPY_TX must also be set to 1 if ioconfigUSE_SPI_ZERO_COPY_TX is set to 1
#endif

#if ( ioconfigINCLUDE_SPI == 1 ) && ( ioconfigUSE_SPI_CIRCULAR_BUFFER_RX == 1 ) && ( ioconfigUSE_CIRCULAR_BUFFER_RX != 1 )
	#error ioconfigUSE_CIRCULAR_BUFFER_RX must also be set to 1 if ioconfigUSE_SPI_CIRCULAR_BUFFER_RX is set to 1
#endif

#if ( ioconfigINCLUDE_SPI == 1 ) && ( ioconfigUSE_SPI_TX_CHAR_QUEUE == 1 ) && ( ioconfigUSE_TX_CHAR_QUEUE != 1 )
	#error ioconfigUSE_TX_CHAR_QUEUE must also be set to 1 if ioconfigUSE_SPI_TX_CHAR_QUEUE is set to 1
#endif

#if ( ioconfigINCLUDE_SPI == 1 ) && ( ioconfigUSE_SPI_RX_CHAR_QUEUE == 1 ) && ( ioconfigUSE_RX_CHAR_QUEUE != 1 )
	#error ioconfigUSE_RX_CHAR_QUEUE must also be set to 1 if ioconfigUSE_SPI_RX_CHAR_QUEUE is set to 1
#endif
